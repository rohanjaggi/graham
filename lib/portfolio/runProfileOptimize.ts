// main orchestrator function for profile-optimise endpoint
...
export async function runProfileOptimize(
  ...
  function applyProfileGuardrails(
    ...
    ctx: ProfileOptimizeContext,
    dataWarnings: string[]) {
        const { investmentHorizon, riskTolerance } = ctx
        
        // Rough thresholds for demonstration
        const highVol = annualVol > 0.25
        const deepDd = maxDrawdown < -0.35

        if (investmentHorizon === '<3y' && riskTolerance === 'CONSERVATIVE') {
            if (highVol || deepDd) {
                dataWarnings.push(
                    'Optimised portfolio may be too volatile or drawdown-heavy for a conservative, <3y horizon profile.'
                )
            }
        }
    }
...
// after weight computation can:
// if (hardConstraints?.max_sector_weight != null) {
//   const bySector = await computeSectorWeights(symbols, weights)
//   const violators: string[] = []
//   for (const [sector, w] of bySector.entries()) {
//     if (w > hardConstraints.max_sector_weight!) {
//       violators.push(`${sector}: ${(w * 100).toFixed(1)}%`)
//     }
//   }
//   if (violators.length) {
//     dataWarnings.push(`Sector caps exceeded: ${violators.join(', ')}`)
//     // For a stricter implementation, throw an error instead of warning.
//   }
// }
...
function SearchGlyph({ active }: { active: boolean }) {
  return (
    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" aria-hidden="true" style={{ color: active ? 'var(--gold)' : 'var(--text-muted)' }}>
        ...
        <div style={{
          ...
          {visibleResults.map((r, i) => (
            ...
            >
              ...
              <span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 8 }}>{query.trim() ? 'Open' : formatRecentSearchLabel(r.updatedAt)}</span>
            </button>
          ))}
        </div>
      )}

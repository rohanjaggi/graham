...
const NAV: { section: string; items: NavItem[] }[] = [
  { section: 'ANALYSIS',  items: [{ label: 'Overview', href: '/protected', icon: 'overview' }, { label: 'Research', href: '/protected/research', icon: 'research' }, { label: 'Technical', icon: 'technical' }] },
...
function SearchGlyph() {
  return (
    <svg width="15" height="15" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      ...
      <path d="M10.2 10.2L13.2 13.2" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
    </svg>
  )
}
...
function CloseGlyph() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      <path d="M4 4L12 12" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
      <path d="M12 4L4 12" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
    </svg>
  )
}
...
function ArrowGlyph() {
  return (
    <svg width="13" height="13" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      ...
      <path d="M9 5L12 8L9 11" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
function Sidebar() {
  ...
  return (
    <aside style={{
      ...
      background: 'linear-gradient(180deg, #0B1220 0%, #0E1628 52%, #0A1120 100%)',
      borderRight: '1px solid rgba(226, 196, 138, 0.08)',
      display: 'flex', flexDirection: 'column',
      padding: '0 14px',
    }}>
      <div style={{ padding: '0 6px 14px', borderBottom: '1px solid rgba(226, 196, 138, 0.08)', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 9.5, color: '#74819A', letterSpacing: '0.13em', marginTop: 5, textTransform: 'uppercase' }}>
      ...
      <nav style={{ flex: 1, paddingTop: 18, overflowY: 'auto' }}>
        {NAV.map(({ section, items }) => (
          <div key={section} style={{ marginBottom: 22 }}>
            <div style={{ fontSize: 9.5, fontWeight: 600, letterSpacing: '0.13em', color: '#60708D', padding: '0 14px 8px', textTransform: 'uppercase' }}>
              {section}
            </div>
  ...
  return (
    ...
    <header style={{
      ...
      <div ref={searchWrapRef} style={{ flex: 1, position: 'relative' }}>
        <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: searching ? 'var(--gold)' : 'var(--text-muted)', pointerEvents: 'none', transition: 'color 0.15s', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}><SearchGlyph /></span>
        <input
          className="input-dark"
          ...
          <div style={{
            ...
            {results.map((r, i) => (
              ...
              >
                ...
                <span style={{ color: 'var(--text-muted)', marginLeft: 8, display: 'inline-flex', alignItems: 'center' }}><ArrowGlyph /></span>
              </button>
            ))}
          </div>
        )}
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <TopBar />
        <TickerTape />
        <main style={{ flex: 1, overflowY: 'auto', padding: 'clamp(20px, 2vw, 32px)' }}>
          {children}
        </main>
      </div>
    </div>

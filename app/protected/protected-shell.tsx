...
const NAV: { section: string; items: NavItem[] }[] = [
  { section: 'ANALYSIS',  items: [{ label: 'Overview', href: '/protected' }, { label: 'Research', href: '/protected/research' }, { label: 'Screener', href: '/protected/qa' }] },
        ...
        {NAV.map(({ section, items }) => (
          <div key={section} style={{ marginBottom: 22 }}>
            ...
            {items.map(({ label, href }) => {
              const icon =
                label === 'Overview' ? '⬡' : label === 'Research' ? '⊕' : label === 'Screener' ? '⊟' :
                label === 'Valuation' ? '⊞' :
                label === 'Portfolios' ? '●' :
                label === 'Optimiser' ? '◎' : '◐'
